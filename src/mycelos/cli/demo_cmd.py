...
def _init_simulation(lang: str) -> None:
    ...
    console.print()
    console.print("  [bold]$ curl -fsSL https://raw.githubusercontent.com/mycelos-ai/mycelos/main/scripts/install.sh | bash[/bold]")
    console.print()
    _pause(0.5)
...
def _security_architecture(lang: str) -> None:
    ...
    console.print()
    content = Text()
    content.append("\n  mycelos-gateway    ", style="bold white")
    content.append("mycelos-proxy\n", style="bold yellow")
    content.append("  ├─ Web UI + Chat   ", style="white")
    ...
    content.append("  Internal Docker network (bearer-token auth)\n", style="dim")
    content.append("  Gateway has NO direct internet route (Phase 1b).\n", style="dim")
    content.append(f"  {_t('security_note', lang)}", style="dim italic")
    console.print(Panel(content, title="[bold red]🔒 Two-Container Architecture[/bold red]",
...
def _getting_started(lang: str) -> None:
    ...
    install_url = "https://raw.githubusercontent.com/mycelos-ai/mycelos/main/scripts/install.sh"
    if lang == "de":
        console.print("  [bold white]Mycelos ist kostenlos und Open Source.[/bold white]")
        console.print()
        console.print("  [bold]Installation (Docker, ein Befehl):[/bold]")
        console.print(f"  [bold cyan]curl -fsSL {install_url} | bash[/bold cyan]")
        _pause(0.3)
        console.print()
        console.print("  [dim]Danach:[/dim]")
        console.print("  [bold cyan]http://localhost:9100[/bold cyan]   [dim]— Web-UI oeffnen[/dim]")
        _pause(0.3)
        console.print("  [bold cyan]mycelos doctor[/bold cyan]         [dim]— Status pruefen[/dim]")
        _pause(0.3)
        console.print("  [bold cyan]mycelos update[/bold cyan]         [dim]— Auf neueste Version updaten[/dim]")
        console.print()
        ...
        console.print()
        console.print("  [bold]Install (Docker, one command):[/bold]")
        console.print(f"  [bold cyan]curl -fsSL {install_url} | bash[/bold cyan]")
        _pause(0.3)
        console.print()
        console.print("  [dim]Then:[/dim]")
        console.print("  [bold cyan]http://localhost:9100[/bold cyan]   [dim]— Open the web UI[/dim]")
        _pause(0.3)
        console.print("  [bold cyan]mycelos doctor[/bold cyan]         [dim]— Health check[/dim]")
        _pause(0.3)
        console.print("  [bold cyan]mycelos update[/bold cyan]         [dim]— Pull the latest image[/dim]")
        console.print()
        console.print("  [bold blue]https://github.com/mycelos-ai/mycelos[/bold blue]")
        console.print()
